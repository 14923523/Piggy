...
void CODE_IN_IWRAM md2_tmap(vertex_t* p, int n, PIGGY_ROM u16* texture, int textsize, framebuffer_t* dst)
{
...
void CODE_IN_IWRAM md2_clipped_poly(piggy_t* piggy, polyclip_t src, int n, u16* skin)
{
  int i;
  polygon_t p;
  ...
  md2_tmap(p, n, skin, 8, piggy->viewport.back);
}
#else
void CODE_IN_IWRAM md2_clipped_poly(piggy_t* piggy, polyclip_t src, int n, u16* skin);
#endif /* __PATCH__MD2_TMAP__ */

void CODE_IN_IWRAM md2_unclipped_poly(piggy_t* piggy, polyclip_t p, int n, u16* skin)
{
  int i;
  ...
  if (n > 2) md2_clipped_poly(piggy, p, n, skin);
}
...
void md2_draw(entity_t* e)
{
  ...
  polyclip_t p;
  int nnodes = 0;
  static short buckets[PIGGY_TRIANGLE_BUCKET_MAX];
  static bucket_node_t nodes[1000];
  static vertex_t verts[1000];
  matrx_t m;
  piggy_t* piggy = e->piggy;
  model_t* model = e->visual.data;
  frame_t* f = (frame_t*)((int)model + model->offsetFrames + model->frameSize * (((piggy->tick/3) % 6)+40));
  ...
  for (i = 0; i < model->numVertices; i++)
  {
    ...
    int x = f2i(m[0][0] * u + m[0][1] * v + m[0][2] * w) + e->x - piggy->camera->x;
    int y = f2i(m[1][0] * u + m[1][1] * v + m[1][2] * w) + e->z - piggy->camera->z;
    int z = f2i(m[2][0] * u + m[2][1] * v + m[2][2] * w) + e->y - piggy->camera->y;
  ...
  {    
    bid = ((e->y - piggy->camera->y - verts[t[i].vertexIndices[0]].z) >> 6) +
      (PIGGY_TRIANGLE_BUCKET_MAX>>1);

    bid = CLAMP(bid, 0, PIGGY_TRIANGLE_BUCKET_MAX - 1);

    nodes[nnodes].next = buckets[bid];
    nodes[nnodes].triangle = i;
    buckets[bid] = nnodes++;
  }
  for (bid = 0; bid < PIGGY_TRIANGLE_BUCKET_MAX; bid++)
  {
    for (i = buckets[bid]; i; i = nodes[i].next)
    {
      ...
      md2_unclipped_poly(piggy, p, 3, skin[0]);
    }
    buckets[bid] = 0;
  }
}
