...
#define PIGGY_STR_TITLE      "Piggy Engine"
#define PIGGY_STR_VERSION    "1.0.0.1"
#define PIGGY_STR_COPYRIGHT  "Copyright (C) 2015 Zandy Who. All Rights Reserved"
#define PIGGY_STR_CAPTION    (PIGGY_STR_TITLE " " PIGGY_STR_VERSION " - " PIGGY_STR_COPYRIGHT)


#define PIGGY_TEXTURE_SKY 9
#define PIGGY_TEXTURE_WINDOW 10
#define PIGGY_TEXTURE_LAVA 4
#define PIGGY_TEXTURE_WATER 31
...
#define CLEARMEM(A) piggy_memset(A, 0, sizeof(*(A)))

/******************************************************************************/

#define PIGGY_CONSTANT_LIGHT i2f(2)
#define PIGGY_TEXTURE_MAX 32
#define PIGGY_ENTITY_MAX 100
#define PIGGY_POLYGON_MAX 16
#define PIGGY_VPOOL_MAX 512
#define PIGGY_BULLET_MAX 16
#define PIGGY_RAY_MAX 40
#define PIGGY_RAY_QUALITY 1
#define PIGGY_CELL_MAX 200
#define PIGGY_MAP_WIDTH 64
#define PIGGY_MAP_HEIGHT 64
#define PIGGY_LIGHT_MIN i2f(1)
#define PIGGY_LIGHT_MAX i2f(63)
#define PIGGY_RECIPROCAL_MIN 0
#define PIGGY_RECIPROCAL_MAX 16384
#define PIGGY_SINTABLE_MAX 2048
#define PIGGY_TRIANGLE_BUCKET_MAX 512
...
#define PIGGY_VCACHE_MAX 4
#else
#define PIGGY_VCACHE_MAX 8
#endif
...
#define PIGGY_TEXGEN_WALL_LR  0
#define PIGGY_TEXGEN_WALL_FB  1
#define PIGGY_TEXGEN_FLAT     2
#define PIGGY_TEXGEN_BASIC    3
...
{
  fb[I*3*PIGGY_VIEWPORT_HEIGHT]=RGB_BLUE(v)<<3;
  fb[I*3*PIGGY_VIEWPORT_HEIGHT+1]=RGB_GREEN(v)<<3;
  fb[I*3*PIGGY_VIEWPORT_HEIGHT+2]=RGB_RED(v)<<3;
}
/******************************************************************************/
...
#define AFFINE_CASE(I) case (I): AFFINE(0); fb2+=3*PIGGY_VIEWPORT_HEIGHT;

#define AFFINE_LOOP  \
for (; i >= 16; i -= 16, fb2 += 48*PIGGY_VIEWPORT_HEIGHT) { \
AFFINE( 0) AFFINE( 1) AFFINE( 2) AFFINE( 3) AFFINE( 4) AFFINE( 5) AFFINE( 6) AFFINE( 7) \
AFFINE( 8) AFFINE( 9) AFFINE(10) AFFINE(11) AFFINE(12) AFFINE(13) AFFINE(14) AFFINE(15) \
} switch (i) { \
AFFINE_CASE(16) AFFINE_CASE(15) AFFINE_CASE(14) AFFINE_CASE(13) \
...
typedef struct
{
  rgb555_t pixels[PIGGY_VIEWPORT_HEIGHT][PIGGY_VIEWPORT_WIDTH];
} framebuffer_t;
...
typedef vertex_t polygon_t[PIGGY_POLYGON_MAX];
typedef vertex_t* polyclip_t[PIGGY_POLYGON_MAX];
...
typedef struct
{
  ...
  u8 auth[32];
  u8 desc[64];
  rom_cell_t cells[PIGGY_MAP_HEIGHT][PIGGY_MAP_WIDTH];
} rom_map_t;
...
typedef struct 
{
  ...
  int vcache_size;  
  vertex_t* vcache[PIGGY_VCACHE_MAX];
} cell_t;

#define CELL_IS_SOLID(A) ((A)->top <= (A)->bot)
#define CELL_MARK(A) ((A)->mark = piggy->mark)
#define CELL_IS_MARKED(A) ((A)->mark == piggy->mark)
#define CELL_UNMARK(A) ((A)->mark = (piggy->mark - 1))
#define CELL_UNMARK_ALL() (piggy->mark++)
...
typedef struct
{
  ...
  int nentities;
  entity_t entities[PIGGY_ENTITY_MAX];

  int nvpool;
  vertex_t vpool[PIGGY_VPOOL_MAX];

  int nbullets;
  entity_t* bullets[PIGGY_BULLET_MAX];

  surface_t surfaces[PIGGY_TEXTURE_MAX];

  sprite_t overlay;

  vec2_t ray_pos[PIGGY_RAY_MAX];
  vec2_t ray_vel[PIGGY_RAY_MAX];
  vec2_t vis[PIGGY_CELL_MAX];
  cell_t cells[PIGGY_MAP_HEIGHT][PIGGY_MAP_WIDTH];
...
#define PIGGY_VERTEX(PIGGY) (&(PIGGY)->vpool[(PIGGY)->nvpool++ & 511])
#define PIGGY_BULLET(PIGGY) ((PIGGY)->bullets[(PIGGY)->nbullets++ & 15])
...
extern void CODE_IN_IWRAM piggy_draw(piggy_t* piggy);
extern void CODE_IN_IWRAM draw_texture(piggy_t* piggy, polyclip_t src, int n, int tid);
extern void CODE_IN_IWRAM draw_poly(vertex_t* p, int n, PIGGY_ROM texture_t texture, framebuffer_t* dst, lua_t lua);
extern int CODE_IN_IWRAM polygon_clip(piggy_t* piggy, vertex_t** dst, vertex_t** src, int n);
extern void CODE_IN_IWRAM draw_clipped_poly(piggy_t* piggy, polyclip_t src, int n, int tid);
...
extern void piggy_light(piggy_t* piggy, int lightx, int lighty);
extern void piggy_clear_entities(piggy_t* piggy);
extern void piggy_load_map(piggy_t* piggy, rom_map_t* map);
extern void piggy_save_map(piggy_t* piggy, rom_map_t* map);
extern void piggy_init_map(piggy_t* piggy);
extern void piggy_init(piggy_t* piggy,
  framebuffer_t* front,
  framebuffer_t* back,
  texture_t* textures,
  palette_t palette,
  lua_t lua);
extern void piggy_tick(piggy_t* piggy);
extern void piggy_putc(int x, int c);
extern void piggy_puts(int x, char* s);
extern entity_t* piggy_entity(piggy_t* piggy, int x, int y, int z, entity_behaviour_t behaviour);
extern void piggy_default_lighting(piggy_t* piggy);
...
extern PIGGY_ROM rom_map_t e1m1;

/******************************************************************************/

extern PIGGY_ROM texture_t textures[PIGGY_TEXTURE_MAX];
extern PIGGY_ROM palette_t palette;
extern PIGGY_ROM lua_t lua;
extern PIGGY_ROM int reciprocal[PIGGY_RECIPROCAL_MAX];
extern PIGGY_ROM int sintable[PIGGY_SINTABLE_MAX];
